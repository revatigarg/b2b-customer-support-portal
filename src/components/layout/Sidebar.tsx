...
import { Search, FileText, LayoutDashboard, BookOpen, Settings, HelpCircle, Plus } from 'lucide-react';
...
export function Sidebar() {
  ...
  return <aside className="fixed inset-y-0 left-0 z-50 w-64 bg-sidebar text-sidebar-foreground border-r border-sidebar-border">
      <div className="flex h-full flex-col">
        ...
        <div className="flex h-16 items-center gap-3 px-6 border-b border-sidebar-border">
          ...
          <div className="flex flex-col">
            <span className="text-sm font-semibold">Partner Experience Cloud Portal</span>
        ...
        <nav className="flex-1 space-y-1 px-4">
          ...
          return <Link key={item.name} to={item.href} className={cn('group flex items-center gap-3 px-3 py-2.5 text-sm font-medium transition-colors', isActive ? 'bg-sidebar-accent text-sidebar-accent-foreground' : 'text-sidebar-foreground/70 hover:bg-sidebar-accent/50 hover:text-sidebar-accent-foreground')}>
                <item.icon className={cn('h-5 w-5 shrink-0', isActive ? 'text-sidebar-primary' : 'text-sidebar-foreground/50 group-hover:text-sidebar-foreground/70')} />
                {item.name}
              </Link>;
        })}
        </nav>
